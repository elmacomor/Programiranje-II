...
public:
	void AddElement(T1 el1, T2 el2) {
		...
		_elementi1 = temp;
		_elementi2 = temp2;
		temp = nullptr;//delete[] temp-ne smijes
		temp2 = nullptr;
	}
	bool operator==(const Rjecnik& obj) {
		if (_trenutno != obj._trenutno || _omoguciDupliranje != obj._omoguciDupliranje)
			return false;
		for (int i = 0; i < _trenutno; i++) {
	...
	Rjecnik<int, int> operator()(int lokOD, int lokDO) {
		Rjecnik<T1, T2> novi;
		for (int i = lokOD; i <= lokDO; i++) {
	...
	Rjecnik(bool omoguciDupliranje = true) {
		...
		_trenutno = 0;//_trenutno=new int(*_trenunto);
		_elementi1 = nullptr;
		_elementi2 = nullptr;
	}
...
public:
	...
	bool operator==(const ZadovoljstvoKupca& obj) {
		return _ocjena == obj._ocjena && _komentariKarakteristika == obj._komentariKarakteristika;
	}
	bool operator!=(const ZadovoljstvoKupca& obj) {
		return !(_ocjena == obj._ocjena && _komentariKarakteristika == obj._komentariKarakteristika);
	}
	...
	friend ostream& operator<<(ostream& COUT, const ZadovoljstvoKupca& obj) {
		COUT << obj._ocjena << " " << obj._komentariKarakteristika;
		return COUT;
	}
};
...
public:
	Osoba(const char* imePrezime = "", Datum datumRodjenja = Datum()) : _datumRodjenja(datumRodjenja) {//heder funckcija
		_imePrezime = GetNizKaraktera(imePrezime);
	}
	Osoba(const Osoba& obj) : _datumRodjenja(obj._datumRodjenja) {//heder funckcija
...
class Kupac :public Osoba {
	...
	void SaljemMail(int bodovi, int ukupno) {
		m.lock();
		cout << "To:" << _emailAdresa;
		cout << "Subject: Osvareni bodovi" << endl;
		cout << "Postovani," << endl;
		cout << "Prilikom posljednje kupovine ste ostvarili" << bodovi << "bodova, tako da trenutno vas ukupan broj bodova iznosi" << ukupno << endl;
...
public:
	...
	void DodajKupovinu(float iznos, ZadovoljstvoKupca z) {
		_kupovine->AddElement(iznos, z);
		int bodovi = iznos / 10;
		if (bodovi > 0) {
		...
		if (bodovi > 5) {
			...
			thread t1(&Kupac::SaljemMail, this, bodovi, GetBodoviUkupno());
			t1.join();
		}
	}
...
void main() {
	...
	*/
	Rjecnik<int, int> rjecnik2 = rjecnik1(2, 5);//-->operator  
	...
	*/
	...
	kupci[0] = new Kupac("Denis Music", Datum(12, 1, 1980), "denis@fit.ba");//polimorfizam
	...
	Kupac* denis = dynamic_cast<Kupac*>(kupci[0]);//osoba virtuelna, obje klase imaju konst kopije i operator=
