...
public:
    ...
    bool operator==(const Kolekcija& obj) {
        for (int i = 0; i < _trenutno; i++) {
            if (_elementi1[i] != obj._elementi1[i] || _elementi2[i] != _elementi2[i]) {
                return false;
            }
        }
        return true;
    ...
    Kolekcija operator()(int pocetak, int kraj) {
        if (pocetak<0 || kraj <= 1 || pocetak >= _trenutno || kraj >= _trenutno || pocetak>kraj) {
            throw exception("Lokacije nisu ispravno proslijedjene");
        }
        Kolekcija nova;
...
public:
    ...
    bool operator==(const Predmet& obj) {
        return strcmp(_naziv, obj._naziv) == 0 && *_ocjene == obj._ocjene;
    }
    ~Predmet() {
    ...
    float GetProsjek() {
        ...
        if (suma == 0)
            return 0;
        return (float)(suma / _ocjene->getTrenutno());
    }
};
class Uspjeh {
    eRazred _razred;
...
class Kandidat {
    ...
    void SaljiMail(eRazred razred, float prosjekRazreda, float prosjekSkolovanja) {
        ...
        cout << "Dosadasnji uspjeh (prosjek) na nivou: " << razred << " iznosi: " << prosjekRazreda << ", a ukupni uspjeh u toku skolovanja iznosi: " << prosjekSkolovanja << endl;
        cout << "Pozdrav\nFIT Team." << endl;
        brava.unlock();
    }
...
public:
    Kandidat(const char* imePrezime, string emailAdresa, string brojTelefona) {
        ...
        _brojTelefona = ValidirajBrojTelefona(brojTelefona)==true?brojTelefona:"NOT_SET";//izvrsiti validaciju broja telefona
    }
...
*/
    bool AddPredmet(eRazred razred, Predmet p, const char* napomena) {
        for (int i = 0; i < _uspjeh.size(); i++) {
            if (_uspjeh[i].GetERazred() == razred) {
                if (_uspjeh[i].GetPredmeti().getTrenutno() >= 5|| p.GetProsjek()<2.5) {
                    return false;
                }
                ...
                _uspjeh[i].GetPredmeti().AddElement(new Predmet(p), napomena);
                float prosjekRazreda = _uspjeh[i].prosjekRazreda();
                float prosjekSkolovanja = ProsjekSkolovanja();
                thread t1(&Kandidat::SaljiMail, this, razred,prosjekRazreda,prosjekSkolovanja);
                t1.join();
                return true;
            }
        }
        Uspjeh novi(razred);
        novi.GetPredmeti().AddElement(new Predmet(p), napomena);
        _uspjeh.push_back(novi);
        float prosjekRazreda = novi.prosjekRazreda();
        float prosjekSkolovanja = ProsjekSkolovanja();
        thread t2(&Kandidat::SaljiMail, this, razred, prosjekRazreda, prosjekSkolovanja);
        t2.join();
        return true;
    }
